...
module.exports = function (jwtString, secretOrPublicKey, options, callback) {
  ...
  if (options.clockTimestamp && typeof options.clockTimestamp !== 'number') {
    return done(new Error.call('clockTimestamp must be a number'));
  }

  var clockTimestamp = options.clockTimestamp || Math.floor(Date.now() / 1000);

  if (!jwtString){
    return done(new Error.call('jwt must be provided'));
  }

  if (typeof jwtString !== 'string') {
    return done(new Error.call('jwt must be a string'));
  }

  var parts = jwtString.split('.');

  if (parts.length !== 3){
    return done(new Error.call('jwt malformed'));
  }

  var hasSignature = parts[2].trim() !== '';

  if (!hasSignature && secretOrPublicKey){
    return done(new Error.call('jwt signature is required'));
  }

  if (hasSignature && !secretOrPublicKey) {
    return done(new Error.call('secret or public key must be provided'));
  }
